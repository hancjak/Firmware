...
int current_temperature_bed2_raw = 0;
float current_temperature_bed2 = 0.0;
...
#endif
#ifdef BED2_MINTEMP
static int bed2_minttemp_raw = HEATER_BED2_RAW_LO_TEMP;
#endif
#ifdef BED2_MAXTEMP
static int bed2_maxttemp_raw = HEATER_BED2_RAW_HI_TEMP;
#endif
...
static void temp_runaway_stop(bool isPreheat, bool isBed);
  ...
  if (extruder<0)
  {
	 ...
	 soft_pwm_bed2 = (MAX_BED_POWER) / 2;
	 bias2 = d2 = (MAX_BED_POWER) / 2;
   }
   else
   {
...
void manage_heater()
{
  ...
  temp_runaway_check(0, target_temperature_bed2, current_temperature_bed2, (int)soft_pwm_bed2, true);
#endif
...
#ifdef PIDTEMPBED
	  pid_input = current_temperature_bed2;

#ifndef PID_OPENLOOP
	  pid_error_bed = target_temperature_bed2 - pid_input;
	  pTerm_bed = cs.bedKp * pid_error_bed;
	  temp_iState_bed += pid_error_bed;
	  temp_iState_bed = constrain(temp_iState_bed, temp_iState_min_bed, temp_iState_max_bed);
	  iTerm_bed = cs.bedKi * temp_iState_bed;
...
#define K2 (1.0-K1)
...
#else 
	  pid_output = constrain(target_temperature_bed2, 0, MAX_BED_POWER);
#endif //PID_OPENLOOP

#ifdef AMBIENT_THERMISTOR
	  if (((current_temperature_bed2 > BED2_MINTEMP) || (current_temperature_ambient < MINTEMP_MINAMBIENT)) && (current_temperature_bed2 < BED2_MAXTEMP))
#else //AMBIENT_THERMISTOR
	  if ((current_temperature_bed2 > BED2_MINTEMP) && (current_temperature_bed2 < BED2_MAXTEMP))
#endif //AMBIENT_THERMISTOR
	  {
		  soft_pwm_bed2 = (int)pid_output >> 1;
	  }
	  else {
		  soft_pwm_bed2 = 0;
	  }
	  ...
	  if ((current_temperature_bed2 > BED2_MINTEMP) && (current_temperature_bed2 < BED2_MAXTEMP))
	  {
		  if (current_temperature_bed2 >= target_temperature_bed2)
		  {
			  soft_pwm_bed2 = 0;
		  }
		  else
		  {
			  soft_pwm_bed2 = MAX_BED_POWER >> 1;
		  }
	  }
	  else
	  {
		  soft_pwm_bed2 = 0;
		  WRITE(HEATER_BED2_PIN, LOW);
	  }
#else //#ifdef BED_LIMIT_SWITCHING
	  // Check if temperature is within the correct band
	  if ((current_temperature_bed2 > BED2_MINTEMP) && (current_temperature_bed2 < BED2_MAXTEMP))
	  {
		  if (current_temperature_bed2 > target_temperature_bed2 + BED_HYSTERESIS)
		  {
			  soft_pwm_bed2 = 0;
		  }
		  else if (current_temperature_bed2 <= target_temperature_bed2 - BED_HYSTERESIS)
		  {
			  soft_pwm_bed2 = MAX_BED_POWER >> 1;
		  }
	  }
...
#ifdef DEBUG_HEATER_BED_SIM
	current_temperature_bed2 = target_temperature_bed2;
#else //DEBUG_HEATER_BED_SIM
	current_temperature_bed2 = analog2tempBed(current_temperature_bed2_raw);
#endif
...
void tp_init()
{
#if MB(RUMBA) && ((TEMP_SENSOR_0==-1)||(TEMP_SENSOR_1==-1)||(TEMP_SENSOR_2==-1)||(TEMP_SENSOR_BED==-1)||(TEMP_SENSOR_BED2==-1))
  ...
  #endif  
  #if defined(HEATER_BED2_PIN) && (HEATER_BED2_PIN > -1) 
    SET_OUTPUT(HEATER_BED2_PIN);
  #endif  
...
#ifdef BED2_MINTEMP
  /* No bed MINTEMP error implemented?!? */
  while (analog2tempBed(bed2_minttemp_raw) < BED2_MINTEMP) {
...
#endif //BED_MINTEMP
#ifdef BED2_MAXTEMP
  while (analog2tempBed(bed2_maxttemp_raw) > BED2_MAXTEMP) {
#if HEATER_BED2_RAW_LO_TEMP < HEATER_BED2_RAW_HI_TEMP
	  bed2_maxttemp_raw -= OVERSAMPLENR;
#else
	  bed2_maxttemp_raw += OVERSAMPLENR;
#endif
  }
...
void bed2_max_temp_error(void) {
	...
	if (IsStopped() == false) {
		SERIAL_ERROR_START;
		SERIAL_ERRORLNPGM("Temperature heated bed switched off. MAXTEMP triggered !");
		LCD_ALERTMESSAGEPGM("Err: MAXTEMP BED2");
	}
#ifndef BOGUS_TEMPERATURE_FAILSAFE_OVERRIDE
	Stop();
#endif
...
void bed2_min_temp_error(void) {
	...
	if (IsStopped() == false) {
		SERIAL_ERROR_START;
		SERIAL_ERRORLNPGM("Temperature heated bed switched off. MINTEMP triggered !");
		LCD_ALERTMESSAGEPGM("Err: MINTEMP BED2");
	}
#ifndef BOGUS_TEMPERATURE_FAILSAFE_OVERRIDE
	Stop();
#endif
...
void adc_ready(void) //callback from adc when sampling finished
{
	...
	current_temperature_bed2_raw = adc_values[ADC_PIN_IDX(TEMP_BED2_PIN)];
#ifdef VOLT_PWR_PIN
...
#endif
#if HEATER_BED2_PIN > -1
  static unsigned char soft_pwm_b2;
#ifdef SLOW_PWM_HEATERS
  static unsigned char state_heater_b2 = 0;
  static unsigned char state_timer_heater_b2 = 0;
#endif 
#endif
...
#endif
#if defined(HEATER_BED2_PIN) && HEATER_BED2_PIN > -1
	soft_pwm_b2 = soft_pwm_bed2;
	if (soft_pwm_b2 > 0) WRITE(HEATER_BED2_PIN, 1); else WRITE(HEATER_BED2_PIN, 0);
#endif
#ifdef FAN_SOFT_PWM
    soft_pwm_fan = fanSpeedSoftPwm / 2;
...
#endif
#if defined(HEATER_BED2_PIN) && HEATER_BED2_PIN > -1
  if (soft_pwm_b2 < pwm_count) WRITE(HEATER_BED2_PIN, 0);
#endif
#ifdef FAN_SOFT_PWM
...
#define MIN_STATE_TIME 16 // MIN_STATE_TIME * 65.5 = time in milliseconds
  ...
  if (slow_pwm_count == 0) {
	...
	if (soft_pwm_b2 > 0) {
		// turn ON heather only if the minimum time is up 
		if (state_timer_heater_b2 == 0) {
			// if change state set timer 
			if (state_heater_b2 == 0) {
				state_timer_heater_b2 = MIN_STATE_TIME;
			}
			state_heater_b2 = 1;
			WRITE(HEATER_BED2_PIN, 1);
		}
	}
	else {
		// turn OFF heather only if the minimum time is up 
