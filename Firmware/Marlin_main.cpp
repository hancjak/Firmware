...
bool gcode_M45(bool onlyZ, int8_t verbosity_level)
{
	...
	if (lcd_calibrate_z_end_stop_manual(onlyZ))
	{
		...
		refresh_cmd_timeout();
		#ifndef STEEL_SHEET
		if (((degHotend(0) > MAX_HOTEND_TEMP_CALIBRATION) || (degBed() > MAX_BED_TEMP_CALIBRATION)) && (!onlyZ) || (degBed2() > MAX_BED_TEMP_CALIBRATION)) && (!onlyZ))
		{
    ...
    case 105 : // M105
    {
      ...
      if(setTargetedHotend(105, extruder)){
        ...
        SERIAL_PROTOCOL_F(degTargetHotend(extruder),1);
		  ...
		  SERIAL_PROTOCOLPGM(" /");
		  SERIAL_PROTOCOL_F(degTargetBed2(), 1);
		#endif //TEMP_BED_PIN
...
if((eSoundMode==e_SOUND_MODE_LOUD)||(eSoundMode==e_SOUND_MODE_ONCE))
	...
	case 860: // M860 - Wait for PINDA thermistor to reach target temperature.
	{
		...
		if ((degTargetBed2() == 0) && (degTargetBed() == 0) && (degTargetHotend(0) == 0)) {
...
void handle_status_leds(void) {
  ...
  if(millis() > stat_update) {
    ...
    #endif
#if defined(TEMP_BED2_PIN) && TEMP_BED2_PIN > -1
	  max_temp = max(max_temp, degTargetBed2());
	  max_temp = max(max_temp, degBed2());
#endif
...
#endif //(EXTRUDERS > 1)
    if ((PRINTER_ACTIVE) || (!degTargetBed2() && !degTargetBed() && !degTargetHotend(0)) || (!safetytimer_inactive_time))
    {
        safetyTimer.stop();
    }
    else if ((degTargetBed2() || degTargetBed() || degTargetHotend(0)) && (!safetyTimer.running()))
    {
        safetyTimer.start();
    }
